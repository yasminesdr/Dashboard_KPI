...
const ChartRenderer = ({
  ...
  sizeField,
  selectedCharts,
  user, // <- this comes from your context or props (req.user)
}) => {
  ...
  if (!chartRef || typeof chartRef.toBase64Image !== "function") {
    alert("No valid chart found to share.");
    return;
  }

  const chartInstance = chartRef.chartInstance;
  const contextChart = chartInstance?.$context?.chart;
  const datasets = contextChart?.config?.data?.datasets || [];
  const labels = contextChart?.config?.data?.labels || [];

  const hasValidData = datasets.some(
    (ds) => Array.isArray(ds.data) && ds.data.length > 0
  );
  const hasValidLabels = Array.isArray(labels) && labels.length > 0;

  if (!hasValidData || !hasValidLabels) {
    alert("Chart must contain both datasets and labels before sharing.");
    return;
  }
  ...
  return (
    <div style={{ marginTop: "30px", display: "grid", gap: "40px" }}>
      <div className="pdf-export-inline">
  <input
    type="text"
  ...
  />
  <button onClick={handleExportPDF}>Export Charts to PDF</button>
</div>
